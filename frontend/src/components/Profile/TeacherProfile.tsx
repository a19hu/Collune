...
const toEditForm = (teacher: TeacherApi): TeacherEditForm => ({
  name: teacher.user.name || teacher.user.username || "",
  email: teacher.user.email || "",
  phone_no: teacher.user.phone_no || "",
  ...
  full_address: teacher.address?.full_address || "",
});
...
const TeacherProfile = () => {
  ...
  const [payments, setPayments] = useState<TeacherSalaryPaymentApi[]>([]);
  const [attendance, setAttendance] = useState<WorkdayAttendanceApi[]>([]);
    ...
    const loadTeacher = async () => {
      ...
      try {
        const [profileResponse, paymentResponse, attendanceResponse] = await Promise.all([
          ...
          getTeacherSalaryPayments(teacherId),
          getTeacherAttendance(teacherId),
        ]);
        if (!isMounted) return;
        setTeacher(profileResponse.teacher);
        setEditForm(toEditForm(profileResponse.teacher));
        setPayments(paymentResponse.salary_payments);
        setAttendance(attendanceResponse.attendance);
      } catch (error: any) {
        if (isMounted) setMessage(error?.message || "Unable to load teacher profile.");
      } finally {
  ...
  const displayName = useMemo(() => teacher?.user.name || teacher?.user.username || "Teacher Profile", [teacher]);
  ...
  const addressLine = teacher?.address
    ? [teacher.address.full_address, teacher.address.city, teacher.address.state, teacher.address.pincode, teacher.address.country].filter(Boolean).join(", ")
    : "";

  const saveTeacher = async (event: FormEvent) => {
    event.preventDefault();
    if (!teacherId || !editForm) return;
    ...
    try {
      ...
      setEditForm(toEditForm(response.teacher));
      setMessage("Teacher profile updated.");
    } catch (error: any) {
      setMessage(error?.message || "Unable to update teacher profile.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const removeTeacher = async () => {
    ...
    try {
      ...
      setMessage(error?.message || "Unable to delete teacher profile.");
    }
  };

  const recordPayment = async (event: FormEvent) => {
    event.preventDefault();
    if (!teacherId || !paymentAmount) return;

    try {
      const response = await createTeacherSalaryPayment(teacherId, {
      ...
      setPaymentReference("");
      setPaymentNotes("");
      setMessage("Teacher salary payment recorded.");
    } catch (error: any) {
      setMessage(error?.message || "Unable to record salary payment.");
    }
  };

  if (isLoading) return <div className="text-sm text-slate-500">Loading teacher profile...</div>;

  if (!teacher) {
    return (
      <div className="space-y-4 text-left">
        <BackButton onClick={() => navigate("/admin/teachers")} label="Back to teachers" />
        <div className="bg-white border border-slate-200 rounded-2xl p-6 text-sm text-slate-500">Teacher profile not found.</div>
      </div>
    );
  }
    ...
    <div className="space-y-5 animate-fade-in text-left">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setEditForm(toEditForm(teacher))} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-indigo-50 text-indigo-700 text-xs font-bold hover:bg-indigo-100">
      ...
      {showDeleteConfirm && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-4 space-y-3">
          ...
          <p className="text-xs text-red-700">This removes the teacher login and linked teacher profile records.</p>
          <div className="flex justify-end gap-2">
            ...
            <button type="button" onClick={removeTeacher} className="px-3 py-2 rounded-xl bg-red-600 text-xs font-bold text-white">Confirm Delete</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-5">
        <aside className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5">
          <div className="flex items-center gap-4">
            <img
              ...
              src={teacher.profile_image || `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(displayName)}`}
              alt={displayName}
              className="w-20 h-20 rounded-2xl object-cover border border-slate-200"
            />
            <div className="min-w-0">
              <h1 className="text-xl font-black text-slate-900 leading-tight">{displayName}</h1>
              <p className="text-[11px] text-slate-500 font-mono truncate">{teacher.employee_id}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 text-xs">
            <MiniStat label="Subject" value={teacher.specialization || "General"} />
            <MiniStat label="Status" value={teacher.status ? "Active" : "Inactive"} />
          ...
          <Panel title="Contact">
            ...
            <Info label="Phone" value={teacher.user.phone_no} />
        ...
        <main className="space-y-5">
          {editForm && (
            <Panel title="Edit Teacher Profile">
              <form onSubmit={saveTeacher} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <EditInput label="Qualification" value={editForm.qualification} onChange={(value) => setEditForm({ ...editForm, qualification: value })} />
                  <EditInput label="Specialization" value={editForm.specialization} onChange={(value) => setEditForm({ ...editForm, specialization: value })} />
                  <EditInput label="Experience" type="number" value={editForm.experience} onChange={(value) => setEditForm({ ...editForm, experience: value })} />
                ...
                <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
                  <button type="button" onClick={() => setEditForm(toEditForm(teacher))} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-100 text-xs font-bold text-slate-700">
                    <X className="w-4 h-4" />
                    Reset
                  </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <Panel title="Teacher Details" icon={<UserRound className="w-4 h-4" />}>
              <DetailGrid>
                ...
                <Info label="Join Date" value={teacher.join_date} />
                <Info label="Salary" value={teacher.salary} />
                <Info label="Created" value={new Date(teacher.created_at).toLocaleDateString()} />
              </DetailGrid>
            </Panel>
            ...
            <Panel title="Work Summary" icon={<BriefcaseBusiness className="w-4 h-4" />}>
              <DetailGrid>
                ...
                <Info label="Updated" value={new Date(teacher.updated_at).toLocaleDateString()} />
              </DetailGrid>
            </Panel>
          </div>
          ...
          <Panel title="Salary Payment Section" icon={<CreditCard className="w-4 h-4" />}>
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
              <DetailGrid>
                <Info label="Monthly Salary" value={teacher.salary} />
