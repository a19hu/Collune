import { createStaff } from "@/src/lib/authApi";
...
const AddStaff = () => {
  ...
  const saveStaff = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage(`Staff created. Login: ${response.login_username} / ${response.login_password}`);
    } catch (error: any) {
      setMessage(error?.message || "Unable to create staff.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in text-left">
      <button onClick={() => navigate("/admin/staff")} className="inline-flex items-center gap-2 text-xs font-bold text-indigo-600">
        <ArrowLeft className="w-4 h-4" />
        Back to support staff
      </button>

      <form onSubmit={saveStaff} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-5">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Add New Support Staff</h1>
          <p className="text-slate-500 text-xs">Create a staff login and operational profile.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Select label="Department" value={department} onChange={setDepartment} options={["Administration", "Finance", "Library", "Security", "Maintenance", "Transport", "IT"]} />
          ...
          <label className="space-y-1 md:col-span-2">
            ...
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center shrink-0">
                {profileImagePreview ? (
                  <img src={profileImagePreview} alt="Staff preview" className="w-full h-full object-cover" />
                ) : (
        ...
        <div className="border-t border-slate-100 pt-5 space-y-4">
          <h2 className="text-xs font-extrabold text-indigo-600 font-mono uppercase tracking-wider">Staff Address</h2>
        ...
        <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
          <button type="button" onClick={() => navigate("/admin/staff")} className="px-4 py-2 rounded-xl bg-slate-100 text-xs font-bold text-slate-700">
            Cancel
          </button>
